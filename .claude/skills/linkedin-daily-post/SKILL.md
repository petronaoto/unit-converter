...
### Seven traps, all of which have bitten
...
Attaching the image: **only the OS clipboard works.**
...
Then click into the composer body and press Ctrl+V.
...
1. Feed → **Start a post**. A Premium upsell modal often intercepts the first click — close it and
   click again. **Use element refs, not coordinates**; the feed shifts and a coordinate click landed
   on a puzzle link once.
2. Type the English body. Paste the image. Append the Japanese section — appending text after the
   image is fine, the image stays attached below the text block.
3. Drag the composer's scrollbar to review the whole post, image included, before posting.
4. Post. Then open the post and add the pre-filled share link as the **first comment**.
5. Drafts auto-restore if the composer is closed, so a mistake is recoverable.
