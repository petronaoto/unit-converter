...
**Document version:** 2.7 (accompanies app v3.9)
...
6. **Zero data harvesting.** No accounts, no cookies, no advertising, no server-side logging of user data. State lives only in the browser (`localStorage`) or in explicitly shared URLs. **Amended in v3.2:** cookieless, aggregate-only Vercel Web Analytics is now permitted, under a hard boundary — analytics may record *that* a tool was used (a fixed slug such as `steam-if97`) and never *what* was entered into it. No input value, no calculation result and no Report-tab text is ever transmitted. Any change to what is measured must ship in the same commit as the Privacy Policy §7 update that discloses it (MARKETING.md §5).
...
| **v3.7** | Aug 2026 | **Selectable two-phase ΔP methods + rebuilt 3D flow animation** (maintainer-requested 2026-08-20). ΔP card gains a TWO-PHASE METHOD selector — `hem` (default, bit-identical to every pre-v3.7 payload and share link) / **Lockhart-Martinelli** (Chisholm C 20/12/10/5 at the phase-alone Re 2300 threshold) / **Müller-Steinhagen-Heck (1986)** / **Friedel (1979)** with a new SURFACE TENSION input (mN/m, shown for Friedel only) — implemented stdlib-only in `dp_calculator.py` (`two_phase_dpdz()`); the method replaces the frictional term only, so static head, fittings and the vel/Re/f readouts stay on the homogeneous basis and the RP 14E / NORSOK screens remain method-independent (test-enforced). The Flow Regime **3D animation was rebuilt**: holdup-scaled layers/films (λ_L), Taylor bubbles with noses/tails and a draining wall film, churn oscillation from j_G/j_L, annular disturbance waves + entrained core, chord-correct stratified/wavy layers with crest spray, flanged pipe, horizon grid, hemisphere/rim lighting, drag-orbit + wheel zoom, GPU disposal, `prefers-reduced-motion` — plus a mode strip: View (Exterior / Cutaway / Inside the pipe), Regime preview override (flagged PREVIEW), Speed (0.25×–2×) and pause, all persisted/shared via `collectInputs()`. Docs: How To Use §17/§19 + release entry `howto-new37` (`docs.howto.b112`/`b113`), Theory §10.1/§10.5 updated, **§10.6** and three Part XIII citations added (`docs.theory.b053`/`b054`), Terms §3 wording, ten languages. New reference **Vector 14**; 358 tests. |
| **v3.9** | Aug 2026 | **The full active LNG fleet** (maintainer-requested 2026-08-25, enabled by the IGU's written permission of 2026-08-24 — E. Minty, Director Communication, granting reproduction of Appendix 3 with full attribution and a link to the original source). New `LNG_FLEET` dataset: 768 compact rows extracted from the report PDF (pp. 129–153) with pdfplumber, page-by-page re-verified by independent agents (804/804 rows, zero field mismatches, mutation-tested checkers) + the 36 featured `LNG_VESSELS` = **all 804 carriers active at end-2025**. One static `grpFleet` optgroup (option list never rebuilt — share-restore rule holds at 805 options), filters / catalogue / share links work fleet-wide, fleet rows render the IGU attribution + report link (`iguSrc` + `LNG_FLEET_SRC_URL`), new vessel type `bunk`. Featured rows keep their own primary sources, photos and capacities (15 capacity disagreements resolved in the primary source's favour). Docs: §16 + Theory §9.4 rewritten, Terms §6 licence statement, LICENSE third-party section, `fleetContext` note; ten languages (9 translations authored per-language, splice-asserted). Tests extended with the LNG_FLEET battery. |
| **v3.8** | Aug 2026 | **Safety-tab pressure inputs: per-field units, Abs/Gauge, General-tab import, one-click defaults** (maintainer-requested 2026-08-22). Every PRV pressure field (P1/P2/Ps/Po/Pa in all five modes) gets its own unit `<select>` (psi · kPa · bar · MPa · Pa · atm · kg/cm², the General card's factor list), an Abs/Gauge toggle (liquid modes default gauge, the rest absolute — the pre-v3.8 meaning) and a ⇩ button importing the General tab Pressure card value; `calcPSV()` reduces each field to psia / kPa abs (psig / kPag for liquid) via `psvPressForApi()`, so the API and Vector 4 are untouched. Card opens pre-filled — gas/vapor with an 8,000 lb/h, M 19, 560 °R, 179.7 psia case → **orifice H** (new **Vector 15**), other modes on Vector 4 — so CALCULATE works with nothing typed. `psvPm` in share state + legacy-SI migration; `psv-units` → `onPsvUnitsChange()` flips psi↔kPa only. Docs: How To Use §21 rewritten + new anatomy/worked-example block (`docs.howto.b114`), release entry `howto-new38` (`b115`/`b116`), Theory **§12.8** (`docs.theory.b055`), three new `safety.psv.importPress*` keys, `hintGauge`/`hintAbsolute` retired, ten languages. 370 tests. |
| **v3.8.1** | Aug 2026 | **Safety-tab follow-up (maintainer feedback 2026-08-22):** the v3.8 defaults were invisible to returning visitors because the saved session restored over them (and USC-only numbers could not give H under SI). Now: unit `<select>` on **every** physical input — W ×3 (lb/h · kg/h · t/h · kg/s · lb/s), T (°R · K · °F · °C), advisory steam T, Q (gal/min · L/min · m³/h · bbl/h · mL/min), µ (cP · mPa·s · Pa·s), vo/v9 (ft³/lb · m³/kg · L/kg) via `PSV_QTY`/`psvQty()`; `psvEnsureDefaults()` resets an incomplete saved panel to the unit-explicit default case on restore (complete cases untouched; liquid accepts P1 *or* Ps) — one click gives H in USC (0.7144 in²) and SI (461.4 mm²). `_psvUnitLabels()` retired to a no-op. Docs §21/§12.8/release note + ten dictionaries; 375 tests. |
| **v3.8.2** | Aug 2026 | **Safety-tab follow-up #2 (maintainer feedback 2026-08-22):** selecting SI turned the default H case into R because the USC↔SI toggle re-labelled psi→kPa / lb/h→kg/h / °R→K but left the figures. The toggle now **converts the figures it re-labels** (`psvRescale()`, temperatures via kelvin; explicit bar/°C/t/h choices untouched), so the shipped case gives H in USC (0.7144 in²) and SI (≈461 mm²). New **↺ Load example** button (`psvLoadExample()`) restores the shipped case in all modes regardless of the saved session. Docs §21/§12.8/release note + ten dictionaries (2 new keys). |
| **v3.8.3** | Aug 2026 | **Safety-tab follow-up #3 (maintainer feedback 2026-08-22):** ↺ Load example (and the restore fallback) reset the card to the USC-written defaults even with SI selected. A reset panel is now re-expressed in the active unit system (`psvExpressPanelInSystem()` — the two sync passes scoped to that panel), so under SI the example reads 3,628.74 kg/h · 311.11 K · 1,238.99 kPa → H 461.4 mm². |
| **v3.8.4** | Aug 2026 | **Hotfix — the flow-regime inclination gate was direction-blind.** Map selection tested `abs(theta_deg) >= 30`, so a pipe running *downhill* at −45° was classified on Hewitt & Roberts' **upflow** map and returned exactly what +45° returned. The vertical map carries no stratified region at all, and downward inclination is what most favours stratified flow — so for a downhill pipe the likeliest regime was structurally unreachable, not merely imprecise. The gate is now directional (`theta_deg >= 30.0`): only upward inclination uses the upflow map, downhill uses Baker, which does carry Stratified. A low-gas downhill case now reads **Stratified** where it read **Bubbly**. Baker is still a horizontal correlation, so θ ≤ −30° sets `downflow_advisory` + `advisory`, appends `⚠ downflow` to the badge and prints an amber line on the figure — nearest available basis, not a correct one. **Vector 3 and every uphill or shallow case are bit-identical.** Found while building the Day 9 campaign post, which is about this gate; written against v3.7 and renumbered on rebase, so the fix predates the v3.8 line and is independent of it. `test_downward_inclination_is_negative_and_still_vertical` had been locking the defect in place and was flipped; four tests now cover the gate, including one pinning the vertical map's *absence* of a stratified region so the decision is revisited if that ever changes. SPECIFICATION.md §11 #14. 382 tests. |

## 5. Current State (v3.8.3)

### Feature inventory

- **9 tabs:** General · Basic Eng · Advanced (v3.6: three sub-tabs — Gas Quality & LNG Cargo · Hydraulics · GT Fuel) · Safety · How To Use · Theory · Terms of Use · Privacy Policy · Report.
...
- **Advanced:** Compositional GHV & Flow (JIS K 2301, 14 components, HHV/LHV/SG/WI/MCP/MW, ISO 6578 LNG density, mass↔vol↔mol flow, **LNG reference-composition presets with live published-value cross-check — v3.3**), **LNG Cargo Estimator (v3.5: 36-vessel sourced catalogue, loaded/delivered t · kNm³ · TBtu)**, Pipe ΔP (Darcy-Weisbach + Colebrook-White + selectable two-phase friction — **HEM default / Lockhart-Martinelli / Müller-Steinhagen-Heck / Friedel, v3.7** — + RP 14E erosion check + **v2.8 Crane TP-410 fittings and NORSOK P-001 line-sizing screen**), Flow Regime (Hewitt & Roberts / Baker maps + **v3.7 mode-selectable 3D animation**: Exterior/Cutaway/Inside views, regime preview, speed/pause).
- **Safety:** API 520 Part I PRV sizing, five modes, API 526 orifice letters; **v3.8:** per-field pressure unit selects with Abs/Gauge basis, ⇩ import from the General tab Pressure card, one-click defaults (gas → orifice H); **v3.8.1:** unit selects on W/T/Q/µ/vo/v9 too, and incomplete saved panels fall back to the default case on restore; **v3.8.2:** the USC↔SI toggle converts the figures it re-labels, plus a ↺ Load example button; **v3.8.3:** a reset panel is expressed in the active unit system.
...
| Steep **downward** two-phase flow has no map of its own — θ ≤ −30° is shown on the horizontal (Baker) map and flagged as out of basis | Open — partially addressed in **v3.8.4**, which stopped sending downhill lines to the *upflow* map (SPECIFICATION.md §11 #14) and made the shortfall visible instead of silent. The real fix is a vertical-downflow map (Oshinowo & Charles 1974 type); it needs sourced boundary coordinates, and inventing them is exactly the failure mode §11 #9 records. See §6 roadmap |
...
| Vertical-**downflow** regime map (Oshinowo & Charles 1974 type), gated at θ ≤ −30° | Med | M | **Raised by v3.8.4, blocked on sourced boundary coordinates.** v3.8.4 stopped downhill lines being classified on Hewitt & Roberts' *upflow* map, but the replacement is Baker — a *horizontal* correlation — so steep downflow is still outside the basis of everything the tool bundles; it is now flagged rather than silent, which is honest but not correct. Researched 2026-08-21: the Oshinowo & Charles map does **not** share axes with either bundled map (it plots √R_v against Fr_TP/A, A = μ_s/(S_L·σ_s³)^¼, so surface tension becomes a required input rather than Friedel-only) and names six downward regimes — bubbly-coring, bubbly-slug, falling film, falling bubbly-film, froth, annular — none of which exist in `REGION_COLORS`, the Three.js switch or the dictionaries. Its data is 25.4 mm tubes, air–water and air–glycerol, μ_L 1–12 cP, ~25 psia, so applying it to a 4-inch hydrocarbon line is an extrapolation the app must state. That makes it a scoped minor release, not a hotfix follow-on. **The boundaries themselves remain unobtained** — both Oshinowo & Charles (1974) and Barnea's unified model (1987) are paywalled, and open sources give the axes and regime names but never the transition lines. They must be transcribed from the figure or a digitized dataset, never reconstructed: §11 #9 is the record of what happens when a plausible-looking number is invented to fill a gap |
...
| Feature | Value | Effort | Status |
|---|---|---|---|
| Per-field pressure unit drop-downs on all eight PRV pressure inputs | High | M | **Shipped v3.8** (SPECIFICATION.md §4.4) |
| Abs/Gauge toggle per pressure field (General-tab pattern) | High | S | **Shipped v3.8** — liquid modes default gauge, others absolute; basis travels in share state (`psvPm`) |
| ⇩ import from the General tab Pressure card | Med | S | **Shipped v3.8** — converts into the target field's unit/basis |
...
2. **Reference-value regression (mandatory before any release).** The reference vectors in CLAUDE.md and SPECIFICATION.md §9 must reproduce exactly — since v2.6, this means in **every enabled language** (all 10; see SPECIFICATION.md §12), not English only.
   ...
   **Since v2.8 this is partly automated** (`pytest`, run on every push and PR — SPECIFICATION.md §13). CI covers Vectors 2 and 3, i18n key parity, and the architectural rules. It does **not** yet cover Vector 1 (JIS), which remains a manual check because the calculation is JavaScript. The numeric path is language-independent apart from `fmtN`'s hardcoded `en-US` locale, so the per-language requirement is really about the UI not breaking — which is what the feature-preservation sweep in step 4 covers, and which CI cannot do.
   ...
   - ΔP default case: ΔP_total ≈ 176.9 kPa (2.34 friction + 174.6 static), Re ≈ 2.20×10⁵, f ≈ 0.0184, V_e ≈ 7.69 m/s (C=100, corrected constant in v2.8). v3.7 methods on the same case: Lockhart-Martinelli 179.43 kPa (φ_L² = 4.270), Müller-Steinhagen-Heck 177.83 kPa, Friedel 178.63 kPa at σ = 10 mN/m (Vector 14).
