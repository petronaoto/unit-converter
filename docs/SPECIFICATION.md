...
**Document version:** 3.0 (describes app v3.9.5)
...
- **Inputs:** `lc-vessel` (static `<select>`, `""` = manual; 36 featured `<option>`s grouped by class **plus the 768 `grpFleet` options added in v3.9** via `data-i18n-label` optgroups — **never rebuilt**, filters only hide/disable, share-restore rule), `lc-cap` m³ (pre-filled by `selectLNGVessel()`, then user-editable and persisted independently), `lc-fill` % (default 98.5; warns above 98.5, IGC Code Ch. 15), `lc-heel` m³ (default 3,000 since v3.6, was 0), `lc-bor` %/day (default 0.10), `lc-days`. Filter pills `lc-filt-t-*` (class) and `lc-filt-c-*` (containment) drive `lcFilt` → `lcFilterVesselOptions()` + `renderLNGCatalogue()`.
- **Search `lc-search` (v3.9.3):** a free-text box above the pills. `lcSearchChanged()` normalises the query with `lcNorm()` (lower-case, NFD accent-strip, plus o-slash / ae / sharp-s / d-stroke, which do not decompose) into the token array `lcSearchQ`; `lcSearchMatch(v)` requires **every** token to appear somewhere in the per-vessel haystack `[name, IMO, owner, builder, prop, year]`, memoised in `LC_HAYSTACK`. It is the **third predicate** alongside `lcTypeMatch` / `lcContMatch`, so the picker (`lcFilterVesselOptions()`) and the catalogue (`renderLNGCatalogue()`) narrow identically and the query joins the catalogue staleness sentinel. `lcMatchingVessels()` drives the live `lc-search-count` (`{n} of {total}`, amber at zero); Enter (`lcSearchKey()`) selects the vessel as soon as exactly one matches, Escape and the `lc-search-clear` button reset it. **Two invariants:** the currently selected `<option>` is never hidden (`|| o.value === keep`) — a hidden selected option renders the select blank in several browsers; and the query is **excluded from `collectInputs()`**, so it never reaches a share link or `STATE_KEY` — `applyInputs()` only assigns `.value` and fires no handler, so a restored query would both desync from `lcSearchQ` and risk hiding the very vessel the link restored. Measured 4.7 ms per keystroke with the catalogue collapsed, 8.7 ms open (804 rows), so no debounce is used.
...
- **Dataset `LNG_VESSELS`** — ONE `JSON.parse` literal, 36 entries `{id (IMO), name, owner, builder, cap, cont, type, prop, year, src, srcUrl, photo, credit, creditUrl}`; extracted verbatim by `tests/test_lng_cargo.py`. **Provenance by design:** every row cites its own public primary page (owner/operator fleet list, shipbuilder release, class-society register, or Wikipedia as last resort) — never an AIS/spotter site and never the IGU report; a featured row never cites the IGU. Coverage: all 8 vessel types, all 4 containment systems, all 9 propulsion families, 12 shipyards.
- **Dataset `LNG_FLEET` (v3.9)** — ONE `JSON.parse` literal of 768 compact rows `[imo, name, owner, builder, cap_m3, cont, type, prop, year]`, expanded at boot to objects with `igu: true` and indexed by `LNG_FLEET_BY_ID`; together with the 36 featured rows it reproduces **the complete Appendix 3 of the IGU World LNG Report 2026** (data: Rystad Energy) — all 804 LNG carriers active at end-2025 — under the **IGU's written permission of 2026-08-24** (E. Minty, Director Communication; conditions: full attribution and a link to the original source). Both conditions render on every fleet entry: `lcRenderVessel()` routes `v.igu` rows to `advanced.lngCargo.iguSrc` + `LNG_FLEET_SRC_URL` (`https://www.igu.org/igu-reports/2026-world-lng-report/`), the on-card note (`advanced.lngCargo.fleetContext`) states the permission, and Terms §6 carries the licence statement. A featured vessel that also appears in Appendix 3 is deduplicated in favour of the featured row, which keeps its primary source's capacity where the two disagree (15 rows). Vessel type adds `bunk` (Bunkering vessel, 1 ship), bucketed under the small-scale filter. The data is not to be re-extracted from the tool; do not add rows from any other rights-reserved compilation without an equivalent grant.
...
- **Catalogue:** `<details id="lc-cat">` (collapsed by default) with grid `lc-cat-grid` rendered from `LNG_VESSELS` (photo cards) **plus, since v3.9, the filtered `LNG_FLEET` rows as compact cells** (escaped; language-sentinel re-render via `lcRenderCatalogueIfStale()`); the count line is plural-aware since v3.9.3 — `lcCountKey(n)` maps the CLDR category from `new Intl.PluralRules(currentLang).select(n)` through the literal map `LC_COUNT_KEYS` (`one`/`few`/`many`/`other` — `catCountOne`/`catCountFew`/`catCountMany`/`catCount`), so a search narrowed to one ship reads "1 vessel" and Russian keeps its three forms (1 судно · 2 судна · 5 судов); the keys are listed literally so the dead-key sweep still reaches them. The count line totals both sets; "Use this vessel" → `selectLNGVessel()`.
- **i18n:** `advanced.lngCargo.*` (91 keys, incl. the four v3.9.3 `search*` keys and the `catCount` plural set incl. `type.*` / `cont.*` maps `LC_TYPE_KEYS` / `LC_CONT_KEYS` so every key appears literally) in all 10 dictionaries; docs `docs.howto.b102`–`b109`, `docs.theory.b050`–`b052` in all 9 non-English dictionaries.

#### 4.3.2 Pipe Delta Pressure (Darcy-Weisbach) — server-backed

- **Inputs** (`dp-*`): `dp-scale`, `dp-id`/`dp-id-unit`, `dp-len`/`dp-len-unit`, `dp-rough`/`dp-rough-unit`, `dp-elev`/`dp-elev-unit`; vapor `dp-v-flow`/`-u`, `dp-v-den`/`-u`, `dp-v-visc`/`-u`; liquid `dp-l-flow`/`-u`, `dp-l-den`/`-u`, `dp-l-visc`/`-u`; erosion C-factor `dp-cfactor` (default 100). **v3.7:** two-phase method selector `dp-tp-method` (static options `hem` default / `lm` / `msh` / `friedel`) and surface tension `dp-sigma` [mN/m, default 10] inside `dp-sigma-wrap`, which `updateTpMethodUI()` shows only while Friedel is selected (registered in `recomputeAll()` so share-link restores re-show it). The client maps a blank/zero `dp-sigma` to 10 mN/m and sends N/m (÷1000), mirroring the `cfactor` pattern.
...
- **Method (server, §5.1):** phase detection → single vapor / single liquid / two-phase; Darcy-Weisbach friction term + hydrostatic term + **fittings term ΣK·ρv²/2 (v2.8)**; Colebrook-White friction factor (iterative); API RP 14E erosional velocity V_e = 1.2199033·C/√ρ_mix (the exact SI form of V_e = C/√ρ in lb/ft³ units; corrected in v2.8 — §11 #9). **v3.7:** the two-phase *frictional* term is computed by the selected correlation — HEM (default), Lockhart-Martinelli (Chisholm C), Müller-Steinhagen-Heck (1986) or Friedel (1979) — while static head, fittings and the reported vel/Re/f stay on the homogeneous no-slip basis (§5.1).

#### 4.3.3 Flow Regime — server-backed visualizer

- **Calculation inputs:** none of its own — reads the ΔP card's `dp-*` fields. Button "VISUALIZE FLOW REGIME" → `calcFlowRegime()`. **v3.7 adds animation-only controls** (no server round-trip; they re-render the cached `lastFlowRegime`): view `fr-3d-view` (`exterior` default / `cutaway` / `inside`), regime preview override `fr-3d-regime` (`auto` default + the eight regime keys), playback speed `fr-3d-speed` (0.25/0.5/1/2), pause button `fr-3d-pause`. All are static option lists inside `<main>`, so they persist and travel in share links via `collectInputs()`.
...
- **Method (server, §5.3):** inclination θ = asin(Δz/L). θ ≥ **+**30° → vertical map (Hewitt & Roberts type, j_G vs j_L); otherwise horizontal map (Baker type, G_G vs G_L). Simplified piecewise-linear region boundaries — **qualitative orientation only**. **The gate is directional as of v3.8.4** (§11 #14): the vertical map is an *upflow* map with no stratified region, so downhill lines are classified on Baker and carry `downflow_advisory: true` plus an `advisory` string, surfaced as an amber line on the figure and a `⚠ downflow` suffix on the badge.
- **3D animation (rebuilt v3.7):** regime-specific particle systems (annular film with disturbance waves and entrained core mist, slug/Taylor bubbles with hemispherical noses, dished tails and a backward-draining wall film, churn with an axial-oscillation term scaled from j_G/j_L, bubbly with void-fraction-scaled loading and slip, horizontal slug/bubble with top-wall migration, stratified/wavy with chord-correct layer geometry and crest spray) in a flanged translucent pipe over a world-horizontal reference grid. Layer depths and film thicknesses scale with the no-slip holdup λ_L; speeds scale from superficial velocities. Exterior/Cutaway views support drag-orbit and wheel zoom (cutaway removes the camera-facing shell half via `thetaStart/thetaLength`); Inside view rides the pipe axis. A manual regime is rendered under the calculated θ/velocities and flagged **PREVIEW** in the overlay. `prefers-reduced-motion` starts the scene paused. Disposes geometries/materials and event listeners on re-run; CDN failure degrades to the 2D map with a notice.

### 4.4 Safety — PRV Sizing (API 520 Part I, 9th Ed.)

- **Mode select** `psv-mode`: gas / steam / liquid_cert / liquid_noncert / twophase. Unit system `psv-units`: USC or SI. `updatePSVMode()` swaps the visible input panel (`psv-inputs-gas`/`-steam`/`-liquid`/`-twophase`) and the W / T / Q / vo / v9 unit labels (`_psvUnitLabels()`). **Since v3.8 the eight pressure fields carry their own unit selects and are not relabelled** — a `psv-units` change goes through `onPsvUnitsChange()`, which first moves any pressure select sitting on psi↔kPa to the other system's canonical unit (a bar / MPa / atm choice is left alone). **v3.8.2: the figure is converted together with the label** (`psvRescale()` — factor ratio; temperatures through kelvin), so the physical case and therefore the orifice are unchanged by the toggle — the former "numbers stay, labels switch" rule turned the default H case into an R case the moment SI was selected and then calls `updatePSVMode()`.
- **Inputs by mode** (server-side fallbacks apply when a field is blank; **since v3.8 every required field ships pre-filled** — gas with the orifice-H case of §9 Vector 15, the other modes with their §9 Vector 4 cases — so CALCULATE ORIFICE SIZE returns a result with nothing typed):

| Mode | API 520 § | Fields (IDs) | Defaults |
|---|---|---|---|
| Gas | §5.6 | `psv-W`, `psv-M`, `psv-k`, `psv-T`, `psv-Z`, `psv-P1`, `psv-P2`, `psv-Kd`, `psv-Kb`, `psv-Kc` | on-screen (v3.8): W 8,000 lb/h, M 19, k 1.3, T 560 °R, Z 1.0, P1 179.7 psi abs, P2 0 psi abs → **orifice H** (Vector 15); server fallbacks M 28, k 1.3, Z 1.0, Kd 0.975, Kb 1.0, Kc 1.0 |
| Steam | §5.7 | `psv-W-steam`, `psv-P1-steam`, `psv-Kd-steam`, `psv-Kb-steam`, `psv-Kc-steam`, `psv-KSH`; **v3.0 advisory-only:** `psv-T-steam` (+`-unit`) and the `psv-steam-advisory` strip | on-screen (v3.8): W 153,500 lb/h, P1 1,774.7 psi abs (Vector 4 → K); Kd 0.975, Kb 1.0, Kc 1.0, KSH 1.0 |
...
| Liquid (certified) | §5.8 | `psv-Q`, `psv-Gl`, `psv-P1-liq`, `psv-P2-liq`, `psv-mu`, `psv-Kd-liq`, `psv-Kw`, `psv-Kc-liq` | on-screen (v3.8): Q 1,800 gal/min, Gl 0.9, µ 0, P1 275 psi gauge, P2 0 psi gauge (Vector 4 → N); server fallbacks Gl 1.0, Kd 0.65, Kw 1.0, Kc 1.0 |
| Liquid (non-certified) | §5.9 | as above + `psv-Ps`, `psv-Kp` | on-screen (v3.8): Ps 250 psi gauge (Vector 4 → N); Kd 0.62, Kp 1.0 |
| Two-phase | §5.10 / Annex C | `psv-W-tp`, `psv-vo`, `psv-v9`, `psv-Po`, `psv-Pa`, `psv-Kd-tp`, `psv-Kb-tp`, `psv-Kc-tp`, `psv-Kv-tp` | on-screen (v3.8): W 238,715 lb/h, vo 0.3116 ft³/lb, v9 0.3629 ft³/lb, Po 80.7 psi abs, Pa 0 (Vector 4 → T); Kd 0.85, Kb/Kc/Kv 1.0; **Pa ≤ 0 → atmospheric server-side** (14.696 psia / 101.325 kPa — v3.0 PR-1, §11 #5; the UI field's literal `value="0"` therefore now means "atmospheric", the physical minimum for a vented system, not "vacuum") |

**Pressure fields (v3.8) — per-field unit, Abs/Gauge basis, General-tab import.** The eight pressure inputs `psv-P1`, `psv-P2` (gas), `psv-P1-steam`, `psv-P1-liq`, `psv-Ps`, `psv-P2-liq` (liquid) and `psv-Po`, `psv-Pa` (two-phase) each have:

- a sibling **unit `<select id="<id>-u">`** whose option values are multiply-to-Pa factors — `6894.75729` psi · `1000` kPa · `100000` bar · `1000000` MPa · `1` Pa · `101325` atm · `98066.5` kg/cm² — a STATIC list (share-link restore sets the value before any event fires) shared with the General tab Pressure card; psi is the HTML default. The former fixed `psv-*-unit` spans are gone.
- an **Abs/Gauge pair** `<id>-abs` / `<id>-gau` (`setPsvPressMode(id, mode, quiet)`, painted by `paintPsvPressMode()`); the basis lives in the `psvPressMode` map, defaulting to gauge for the three liquid fields (`PSV_PRESS_GAUGE_DEFAULT` — API 520 §5.8/§5.9 are written in gauge terms) and absolute elsewhere, i.e. exactly the pre-v3.8 meaning of each field. A toggle marks the result stale, re-runs the steam advisory for `psv-P1-steam`, and schedules a save.
- a **⇩ import button** → `importPsvPressure(id)`: reads the General tab Pressure card (Unit A `press-input1`/`press-select1`/`pressMode1`, falling back to Unit B), converts through `pressToAbsPa()`/`pressFromAbsPa()` into the *target field's* unit and basis (never overwrites the target's select or toggle), writes `formatValuePlain()`, dispatches an `input` event (stale marker, autosave, advisory) and toasts `safety.psv.importPressDone` / `importPressEmpty`. Title/aria key `safety.psv.importPressTitle`.

`calcPSV()` reads every pressure through **`psvPressForApi(id, units, gaugeOut)`** = `P_field × f_unit / f_sys` ± `P_atm / f_sys` with `f_sys` = 6,894.75729 (USC) or 1,000 (SI) and `P_atm = PATM_PA` = 101,325 Pa — i.e. psia / kPa abs for gas, steam and two-phase (`gp()`), psig / kPag for the liquid modes (`gg()`). When the field already sits on the system's canonical unit and basis the raw value is passed through (no round trip), so a stock case reproduces the pre-v3.8 payload byte-for-byte and **the API contract (§5.2) and Vector 4 are untouched**. `updateSteamAdvisory()` reads the same converted figure. The pre-check `missing` list is evaluated on the converted values. Persistence: `collectState()` adds `psvPm` (the basis map); `applyState()` restores it and, for a **pre-v3.8 payload** (no `psv-P1-u` key) whose `psv-units` is `SI`, calls `psvSyncPressUnits('SI')` so the restored kPa figures keep their meaning. i18n: three new `safety.psv.importPress*` keys in all 10 dictionaries; `safety.psv.hintGauge` / `hintAbsolute` removed (the toggle replaces the fixed hints); `safety.psv.twophaseInfoBox` reworded. Pinned by `tests/test_psv_pressure_inputs.py`. Theory §12.8 states the basis rule; How To Use §21 carries the field anatomy and the one-click worked example.

**Quantity fields (v3.8.1) — unit select on every other physical input.** `psv-W`, `psv-W-steam`, `psv-W-tp` (lb/h `0.45359237` · kg/h `1` · t/h `1000` · kg/s `3600` · lb/s `1632.932532` — multiply-to-kg/h), `psv-T` (codes `R` · `K` · `F` · `C`), `psv-T-steam` (`F` · `C` · `K` · `R`, advisory only — still never in the payload; `tests/test_steam_if97.py` pins the five references), `psv-Q` (gal/min `3.785411784` · L/min `1` · m³/h `16.66666667` · bbl/h `2.649788249` · mL/min `0.001` — multiply-to-L/min), `psv-mu` (cP `1` · mPa·s `1.0` · Pa·s `1000` — string-distinct, multiply-to-cP), `psv-vo`/`psv-v9` (ft³/lb `0.062427961` · m³/kg `1` · L/kg `0.001`). Each has a sibling `<select id="<id>-u">` (STATIC list; first option = USC canonical, second = SI canonical) and an entry in **`PSV_QTY`** (`usc`/`si` canonical values, `temp: true` for the affine temperatures). **`psvQty(id, units)`** returns the figure in the API's canonical unit for the active system — pass-through when already there, otherwise `v × f / F` (or through kelvin for temperatures); `calcPSV()` reads W / T / Q / µ / vo / v9 through `gq()` and `updateSteamAdvisory()` reads the advisory temperature the same way. `onPsvUnitsChange()` calls `psvSyncQtyUnits()` after `psvSyncPressUnits()` (canonical ↔ canonical only; since v3.8.2 the figures are converted with the label). `_psvUnitLabels()` is now an intentional no-op (nothing left to relabel; kept as the historical hook). Dimensionless inputs (M, k, Z, Gl, the K-factors, KSH) keep their fixed chips.

**Restore-time default fallback (v3.8.1).** A saved session restores over the HTML `value=` defaults, which is why a returning visitor never saw the one-click case (and an SI visitor could not have reached H with USC-only numbers). `applyState()` now calls **`psvEnsureDefaults()`** after `applyInputs()` and before `recomputeAll()`: for each panel, if the required figures (`PSV_PANEL_REQUIRED` — gas W/M/k/T/P1; steam W/P1; liquid Q/Gl and **P1 *or* Ps**; two-phase W/vo/v9/Po) are not all present, `psvResetPanel()` restores the shipped defaults for that panel — inputs (`defaultValue`), unit selects (`defaultSelected`) and Abs/Gauge basis alike. A complete saved case is left exactly as saved. **`psvLoadExample()` (v3.8.2, button `psv-load-example` "↺ Load example" in the card header, keys `safety.psv.loadExample`/`loadExampleTitle`)** resets all four panels to the shipped case on demand, whatever the saved session holds — mode and unit system untouched. **v3.8.3:** every panel that is reset (by the button or by the restore fallback) is then passed through `psvExpressPanelInSystem()` — `psvSyncPressUnits(units, box)` + `psvSyncQtyUnits(units, box)` scoped to that panel — so under SI the example reads 3,628.74 kg/h · 311.11 K · 1,238.99 kPa (same physical case, still H) instead of flipping the card back to USC labels. The defaults are unit-explicit (8,000 **lb/h**, 560 **°R**, 179.7 **psi abs** …), so one click gives orifice H in either unit system (SI: 461.4 mm² → H 506.5 mm²).
...
- **Privacy Policy** — 10 clauses (zero collection, localStorage-only state, stateless APIs, hosting, report feature, no cookies/tracking, children, rights, contact). **v3.9.4:** §5 additionally discloses the How To Use demo-video embed — a click-to-play placeholder whose `youtube-nocookie.com` iframe is created by `loadDemoVideo()` only after the user presses play; no request reaches YouTube on page load (EN inline + `docs.privacy.b007` in the 9 dictionaries). The same video (<https://youtu.be/TX4Xp0RhQ6s>) is linked from a `common.demoLink` pill at the top of the General, Basic Eng, Advanced and Safety tabs. **v3.9.5:** the complete-walkthrough tutorial (<https://youtu.be/yCEfv4oiMwc>) is linked the same way — a second pill (`common.tutorialLink`) and a second click-to-play card (`howto-tutorial`, `loadTutorialVideo()`), side by side with the demo in `howto-videos`; §5 of the Privacy Policy covers both videos.
...
{ "scale": 1, "id": 4, "id_mult": 0.0254, "len": 100, "len_mult": 1,
  ...
  "cfactor": 100, "k_total": 0, "tp_method": "hem", "sigma": 0.010 }
```
...
`tp_method` (v3.7, **optional, default `"hem"`**) selects the two-phase frictional correlation: `hem` | `lm` | `msh` | `friedel`. An absent/empty value means `hem`, which reproduces every pre-v3.7 payload and share link bit-for-bit — the same load-bearing-default pattern as `k_total` and `cfactor`. A **present** unknown value is a structured `Invalid Input` error, never a silent fallback. `sigma` (v3.7, **optional, default 0.010 N/m**) is the gas-liquid surface tension consumed by Friedel's Weber number; a present non-positive/non-finite/non-numeric value is rejected regardless of method (the cfactor contract). Friedel additionally requires μ_v < μ_l (its `(1−μ_v/μ_l)^0.7` term) and rejects otherwise.
...
**v3.7 additions — all additive; no field was renamed or removed.**

| Field | Meaning |
|---|---|
| `tp_method` | The validated method key (`hem`/`lm`/`msh`/`friedel`), echoed even for single-phase flows (where it has no effect) |
| `sigma` | Surface tension as applied [N/m], echoed for traceability |
| `phi2` | Two-phase multiplier actually applied — φ_L² (vs liquid-alone) for `lm`, φ_LO² (vs all-flow-as-liquid) for `friedel`; absent for `hem`/`msh` |
| `lm_X` / `lm_C` | Martinelli parameter X and Chisholm C (20/12/10/5 by the phase-alone regimes, threshold Re 2300); `lm` only |

The two-phase badge becomes `"Two-Phase (Lockhart-Martinelli)"` / `"Two-Phase (Müller-Steinhagen-Heck)"` / `"Two-Phase (Friedel)"` under the alternative methods; the default stays exactly `"Two-Phase (HEM)"`.
...
**v3.7 two-phase methods (`two_phase_dpdz()`)** — replace ΔP_fric only, for two-phase flows with `tp_method ≠ hem`; every phase-alone and whole-flow friction factor reuses the same Colebrook-White function at the actual pipe relative roughness (an engineering adaptation of correlations originally fitted with smooth-pipe Blasius factors):
- **`lm`** Lockhart-Martinelli (1949) + Chisholm (1967): (dP/dz)_L and (dP/dz)_G with each phase flowing alone in the full bore; X² = (dP/dz)_L/(dP/dz)_G; φ_L² = 1 + C/X + 1/X² with C = 20 (tt) / 12 (vt) / 10 (tv) / 5 (vv) by the phase-alone Re at threshold 2300; (dP/dz)_TP = φ_L²·(dP/dz)_L.
- **`msh`** Müller-Steinhagen & Heck (1986): A = (dP/dz)_LO, B = (dP/dz)_GO (whole mass flux as liquid/gas); (dP/dz)_TP = [A + 2(B−A)x]·(1−x)^(1/3) + B·x³.
- **`friedel`** Friedel (1979): φ_LO² = E + 3.24·F·H/(Fr^0.045·We^0.035) with E = (1−x)² + x²·(ρ_L f_GO)/(ρ_G f_LO), F = x^0.78·(1−x)^0.224, H = (ρ_L/ρ_G)^0.91·(μ_G/μ_L)^0.19·(1−μ_G/μ_L)^0.7, Fr = G²/(g·D·ρ_h²), We = G²·D/(σ·ρ_h), ρ_h the no-slip density; (dP/dz)_TP = φ_LO²·(dP/dz)_LO.

Static head, fittings and vel/Re/f stay on the homogeneous basis under every method, keeping the RP 14E and NORSOK screens method-independent (enforced by `test_method_changes_only_the_frictional_term`).
...
**Directional gate (v3.8.4, §11 #14).** The comparison is on the signed θ, not `abs(θ)`. Downward flow at θ ≤ −30° additionally returns `downflow_advisory: true` and `advisory` (a fixed English string, `DOWNFLOW_ADVISORY`); `process()` appends `| ⚠ downflow` to the badge and `render()` prints the advisory as an amber line above the citation, shortening the plot area by 0.055 of figure height per advisory line so it cannot overprint the x-axis label. Uphill and shallow cases are byte-identical to v3.7 — Vector 3 is unchanged.
...
- **State shape** (v2, since v2.8): `{ "v": 2, "inputs": { "<element-id>": "<value>", … }, "mods": [{ "id", "t", "u1", "u2", "f" }, …], "hv": "hhv|lhv", "fa": "vol|mol", "fb": "mass|mol", "p1": "abs|gau", "p2": "abs|gau", "psvPm": { "<psv-pressure-field-id>": "abs|gau", … }, "tab": "<tab-name>", "lang": "<language-code>" }` — `inputs` covers every `input`/`select`/`textarea` with an id inside `<main>`, **except `report-*`** (v2.8, see below). `lang` was added in v2.6 (purely additive); `psvPm` in v3.8 (purely additive — absent in older payloads, see §4.4 for the legacy-SI migration). `v` was emitted as `1` from v2.4 onward but never read by anything; **v2 is the first version that carries meaning.**
...
**Vector 14 — two-phase frictional correlations (added v3.7).** Vector 2's hydraulics (x = 0.020134, G = 255.256 kg/s·m², ρ_ns = 251.689 kg/m³, static head 174.590 kPa in every case) driven through each `tp_method` (`tests/test_dp_calculator.py`; pins pre-computed independently before implementation):

| Method | Diagnostics | ΔP_fric | ΔP_total |
|---|---|---|---|
| `hem` (and absent — bit-identical) | Re 2.201×10⁵, f 0.01835 | **2.338 kPa** | **176.929 kPa** |
| `lm` | Re_l 2.118×10⁵ / Re_g 4.351×10⁴ (both turbulent) → C = 20, X = 6.1663, φ_L² = 4.2697 | **4.843 kPa** | **179.433 kPa** |
| `msh` | A = 11.791 Pa/m, B = 529.67 Pa/m | **3.243 kPa** | **177.833 kPa** |
| `friedel` (σ = 0.010 N/m) | E = 0.9783, F = 0.0473, H = 21.088, Fr = 1.0320, We = 2630.2, φ_LO² = 3.4294 | **4.044 kPa** | **178.634 kPa** |

Also pinned: dpStatic/dpFittings/vel/Re/f/rho_mix/v_ero/ero_ratio/L_eq are bit-identical across all four methods; the Chisholm C ladder 20/12/10/5 across the four phase-alone regime combinations; φ_LO² increases with σ (We^−0.035); Friedel rejects μ_v ≥ μ_l; unknown `tp_method` and invalid `sigma` return the structured-error superset; single-phase results ignore the method.

**Vector 15 — PRV card one-click default (added v3.8).** Gas/vapor, USC, the values the Safety card opens with: W = 8,000 lb/h, M = 19 g/mol, k = 1.3, T = 560 °R, Z = 1.0, P1 = 179.7 psi abs, P2 = 0 psi abs, Kd 0.975 / Kb 1.0 / Kc 1.0 → C = **346.9764**, Pcf = **98.067 psia** ≥ P2 (critical), A = **0.7144 in²** → **orifice H** (0.785 in²). Entered as 165 psi *gauge* (+101,325 Pa = 179.696 psia) the letter is unchanged. The steam, liquid and two-phase panels open on the Vector 4 cases (→ K / N / N / T). Pinned by `tests/test_psv_pressure_inputs.py`, which also asserts the on-screen `value=` attributes are these numbers and that How To Use §21 quotes them.
...
| 14 | ~~Flow-regime map selection tested `abs(theta_deg) >= 30`, so a **downhill** line was classified on Hewitt & Roberts' *upflow* map~~ — θ = −45° returned exactly what θ = +45° returned | `api/flowregime.py` | **FIXED v3.8.4.** Found while preparing the Day 9 campaign post, which is about this very gate. The vertical map's regions are Bubbly, Churn/Slug, Wavy, Annular and Disperse — **there is no stratified region on it anywhere** — and downward inclination is precisely what most favours stratified flow, so for a downhill pipe the likeliest regime was unreachable no matter how good the boundaries were. Not a boundary-accuracy quibble: a structural impossibility. The gate is now directional (`theta_deg >= 30.0`); downhill goes to Baker, which does carry Stratified. Demonstration: a low-gas downhill case (θ = −45°, G_G 0.171, G_L 68.5 kg/s·m²) now reads **Stratified** where it previously read **Bubbly**. Baker is still a *horizontal* correlation, so θ ≤ −30° also sets `downflow_advisory` + `advisory` and is flagged on the figure — nearest available basis, not a correct one; a true vertical-downflow map (Oshinowo & Charles 1974 type) is the real fix and is on the DEVELOPMENT_PLAN roadmap. **Vector 3 is bit-identical** (uphill is untouched). `test_downward_inclination_is_negative_and_still_vertical` was a defect-locking test — it asserted the wrong behaviour — and was flipped to `test_the_gate_is_directional_not_symmetric`, joined by `test_downflow_is_flagged_as_out_of_basis`, `test_the_gate_boundary_is_inclusive_upward_only` and `test_the_vertical_map_has_no_stratified_region`, which pins the structural reason so that adding a stratified region to the vertical map forces this decision to be revisited |
...
**Not yet localized (server side):** the three Python endpoints (§5) still return English prose for status/error text. `flowregime.py` already returns a machine-readable `regime_key` alongside its English `regime` label (§5.3) — the ~10+ other message/error branches across the three files are unkeyed. **v3.8.4 adds one more:** the `advisory` string (`DOWNFLOW_ADVISORY`) and the `⚠ downflow` badge suffix are English regardless of UI language. The boolean `downflow_advisory` is the machine-readable half, so a Milestone 4 pass needs only to key the prose — nothing branches on the English text. See DEVELOPMENT_PLAN.md §6 Milestone 4 (optional).
...
| `tests/test_dp_calculator.py` | Vectors 2, 7 and 14 (v3.7 two-phase methods), unit-factor guards, friction-factor and phase-detection branches |
| `tests/test_psv_calculator.py` | Vector 4 candidates (§9), the five sizing modes, API 526 orifice selection |
| `tests/test_psv_pressure_inputs.py` | (v3.8 / v3.8.1) Vector 15 through the real module; the quantity selects (`PSV_QTY` ↔ `<select>` canonical pairs, string-distinct viscosity, `gq()` routing), `psvEnsureDefaults()` ordering and the P1-or-Ps liquid rule; + the Safety-tab pressure-field contract: eight `psv-*-u` selects with the static factor list shared with the General card, Abs/Gauge buttons and their defaults, `psvPressForApi()` routing in `calcPSV()`, `psvPm` state + legacy-SI migration, the one-click defaults and the §21 worked example |
...
| `tests/test_lng_cargo.py` | (v3.5) Vector 13 chain pins + page↔spec worked-example agreement; the named unit constants (`GJ_PER_MMBTU`, `MJ_PER_MWH`, `KG_PER_LB`, `RHO_LBSCF_TO_KGNM3` derived not typed) and the lb/scf-not-lb/ft³ guard on `gt-rho-u` / `importGHVToGT()` / `gtLastResult.rho`; GT totals default units unchanged + the new `gt-tot-*-q` column blanked on bad input; Vector 11 through the switched units (27.394 Bscf, 28.00 / 12.32 TBtu, 13.00 PJ, 3,474.2 MMBtu/h, 3.40 MMscf); LNG_VESSELS extraction (JSON.parse contract), 36 unique IMOs, full class/containment/propulsion coverage; **provenance** — every row has a public non-AIS, non-IGU source URL; **photos** — every `photo` flag ↔ file ↔ CREDITS.json entry with a CC/PD licence and matching on-screen credit, ≤ 130 KB, no orphans, README disclaims MIT; static-option-list completeness; **(v3.9) `LNG_FLEET` extraction — 768 rows + 36 = 804, disjoint IMOs, typed domains, name-sorted, wrap-artifact-free, option↔dataset label equality, rendered IGU attribution + link + no-re-extraction undertaking, curated rows still never cite the IGU**; `lastLNGProps` bridge and untouched `lastGHV`; no JIS/ISO 6578 recomputation in the card; wiring into `recomputeAll`/`calcGHV`/`calcGTFuel`; no innerHTML in the vessel panel; escaped catalogue; IMO-only outbound links; `'lc': 'lng-cargo'` in the analytics map; export §7; **(v3.9.3)** search as the third predicate in both consumers, the never-hide-the-selection invariant, the query in both sentinel expressions, `lc-search` excluded from `collectInputs()`, the searched fields and diacritic folding, and the four `search*` keys present in all ten dictionaries; the plural-rule count (`Intl.PluralRules` wiring, `{n}` surviving in all four forms, Russian distinct in three, the non-pluralising five collapsing to one) |
