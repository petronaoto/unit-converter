...
- **Catalogue:** `<details id="lc-cat">` (collapsed by default) with grid `lc-cat-grid` rendered from `LNG_VESSELS` (photo cards) **plus, since v3.9, the filtered `LNG_FLEET` rows as compact cells** (escaped; language-sentinel re-render via `lcRenderCatalogueIfStale()`); the count line totals both sets; "Use this vessel" → `selectLNGVessel()`.
- **i18n:** `advanced.lngCargo.*` (88 keys, incl. the four v3.9.3 `search*` keys incl. `type.*` / `cont.*` maps `LC_TYPE_KEYS` / `LC_CONT_KEYS` so every key appears literally) in all 10 dictionaries; docs `docs.howto.b102`–`b109`, `docs.theory.b050`–`b052` in all 9 non-English dictionaries.
