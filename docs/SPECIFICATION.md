...
**Document version:** 2.1 (describes app v3.7)
...
- **Method (server, §5.3):** inclination θ = asin(Δz/L). θ ≥ **+**30° → vertical map (Hewitt & Roberts type, j_G vs j_L); otherwise horizontal map (Baker type, G_G vs G_L). Simplified piecewise-linear region boundaries — **qualitative orientation only**. **The gate is directional as of v3.7.1** (§11 #14): the vertical map is an *upflow* map with no stratified region, so downhill lines are classified on Baker and carry `downflow_advisory: true` plus an `advisory` string, surfaced as an amber line on the figure and a `⚠ downflow` suffix on the badge.
...
**Directional gate (v3.7.1, §11 #14).** The comparison is on the signed θ, not `abs(θ)`. Downward flow at θ ≤ −30° additionally returns `downflow_advisory: true` and `advisory` (a fixed English string, `DOWNFLOW_ADVISORY`); `process()` appends `| ⚠ downflow` to the badge and `render()` prints the advisory as an amber line above the citation, shortening the plot area by 0.055 of figure height per advisory line so it cannot overprint the x-axis label. Uphill and shallow cases are byte-identical to v3.7 — Vector 3 is unchanged.
