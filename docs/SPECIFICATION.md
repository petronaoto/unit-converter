...
- **How To Use** — Operations manual: "What's New" blocks (v2.8, v2.5, v2.4) + 16 illustrated sections (header/nav, General, custom modules, Basic Eng, Advanced §1–4, ΔP, Flow Regime, PRV, Report, Gas Property Estimator, fittings & line sizing, mobile navigation & shared modules — and, added v3.0, Steam Properties) using CSS wireframe diagrams. Since v2.5: jump-link strip at the top and section anchors (`howto-new28`, `howto-new25`, `howto-new`, `howto-1`…`howto-16`).
- **Theory** — Constants and formulas: Part I gas compositional (JIS K 2301 with worked examples), Part II density/flow, Part III LNG Klosek-McKinley (Tables B.2/C), Part IV hydraulics (Papay Z-factor §4.1, Darcy-Weisbach + Colebrook §4.2, flow-regime maps §4.3, erosional velocity §4.4), Part VI PRV sizing (§6.1–§6.7 incl. API 526 orifice table), Part V data sources, Part VII real-gas properties & fittings, Part VIII steam (IAPWS-IF97 §8.1–§8.5, added v3.0, anchor `theory-p8`). Worked-example numbers must match actual calculator output exactly.
...
- **Share-link length warning (v2.8).** A stock link is already ~4.1 kB (169 input keys as of the v3.0 cycle → ~3,000 JSON bytes → 4,075 URL chars, measured) and each custom module adds ~60–120 bytes. Browsers cope — the fragment never reaches the server — but QR generators, Outlook/Teams link rewriting and several chat clients truncate around 2,000 characters, producing a link that looks fine and restores nothing. Past `SHARE_URL_WARN_LEN` (2,000) the copy toast says so. It warns rather than blocks: the link is still valid and the user knows where it is going.
...
All vectors are enforced automatically on every push and pull request, except Vector 1 and the display layer of Vector 8 (browser-side JavaScript) — see §13.
...
- `i18n/ja.json`, `zh.json`, `ko.json`, `th.json`, `id.json`, `ru.json`, `es.json`, `fr.json`, `de.json` — each carries the full 404-key working-tool set (368 through v2.8.1 + 36 steam keys in the v3.0 cycle) **plus** the 143 `docs.*` documentation keys (125 shipped in v2.7; 14 v2.8 blocks translated in v2.8.1; 4 steam blocks added with their translations in the same PR-2 commit).
...
- **`data-i18n-html` attributes** mark 143 block-level elements across the four documentation tabs (`docs.howto.b001`–`b078`, `docs.theory.b001`–`b041`, `docs.terms.b001`–`b010` + `langNote`, `docs.privacy.b001`–`b012` + `langNote`). v2.8 added 14 (howto `b069`–`b076`, theory `b034`–`b039`), translated in all 9 non-English dictionaries as of v2.8.1 — see §12.5.1. The v3.0 cycle (PR-2) added 4 (howto `b077`/`b078`, theory `b040`/`b041`) with all 9 translations landing in the same commit — the first feature PR to ship with zero translation debt. New keys are appended numerically even when the section sits near the top of the DOM, because the number is only an identifier and renumbering would touch every dictionary. Blocks were chosen so that **no element containing an `id` (section anchors, jump-link targets) is ever inside a swapped region** — anchors, the back-to-top behaviour, and wireframe structure are untouched by language switches.
