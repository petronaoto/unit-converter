...
| 11 | Z-Factor Estimator silently ignores a temperature of exactly **0**. Its guard is `if(!sg \|\| !p \|\| !t) return;`, and `0` is falsy — so entering 0 °C (or 0 °F) leaves the *previous* result on screen rather than recomputing. 0 °C is a perfectly ordinary process temperature, and the stale value gives no indication it is stale. Confirmed empirically in a browser: `z-result` retained a sentinel value after `calcZFactor()` with `z-t = 0`, `z-t-u = C`. The v2.8 Gas Property Estimator deliberately uses `isNaN(t)` instead and accepts 0 °C correctly (Z = 0.6934 at SG 0.65 / 2,000 psi) — so the two cards now disagree at exactly one input | `index.html` `calcZFactor()` | Logged v2.8. **Not fixed** — changing a guard in a shipped calculator is a behavioural change needing the maintainer's approval, per the preservation rules. One-line fix available |
