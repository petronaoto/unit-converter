...
**Document version:** 1.8 (describes app v3.4)
...
- **How To Use** — Operations manual, 21 illustrated sections in tab order, using CSS wireframe diagrams: global (1 header & tab navigation, 2 mobile navigation & sharing, 3 unit-aware copy) → General (4 standard converter cards, 5 custom modules) → Basic Eng (6 the nine-card overview, 7 Gas Property Estimator, 8 Steam Properties, 9 NPSHa, 10 Compressor) → Advanced (11 gas composition input, 12 LNG reference compositions, 13 operating conditions, 14 physical & combustion properties, 15 mass/volumetric flow, 16 pipe ΔP, 17 fittings & line sizing, 18 Flow Regime) → Safety (19 PRV sizing) → GT Fuel (20) → Report (21), followed by **`Appendix — Release Notes`** (anchor `howto-releases`): the "What's New" changelogs for v3.3, v3.2, v3.1, v3.0, v2.8, v2.5 and v2.4, one collapsed `<details>` each (latest `open`), anchors `howto-new33`, `-new32`, `-new31`, `-new30`, `-new28`, `-new25`, `howto-new`. Since v2.5: jump-link strip at the top and section anchors `howto-1`…`howto-21` — **reassigned in v3.4** to match the new order.
...
- **Theory** — Constants and formulas, Parts I–XII in tab order: Part I real-gas properties for Basic Eng (Papay Z §1.1, LGE viscosity §1.2, sonic velocity §1.3, Joule-Thomson §1.4, anchor `theory-p1`), Part II steam IAPWS-IF97 (§2.1–§2.5, `theory-p2`), Part III pump-suction NPSHa (§3.1–§3.3, `theory-p3`), Part IV compressor head & power (§4.1–§4.3, `theory-p4`), Part V gas compositional analysis per JIS K 2301 with worked examples (§5.1–§5.7, `theory-p5`), Part VI LNG reference compositions (§6.1–§6.5, `theory-p6`), Part VII standard gas density & flow conversions (§7.1–§7.2, `theory-p7`), Part VIII LNG liquid density, Klosek-McKinley Tables B.2/C (§8.1–§8.3, `theory-p8`), Part IX pipe hydraulics for the Advanced ΔP card (Darcy-Weisbach + Colebrook §9.1, Crane TP-410 fittings §9.2, NORSOK P-001 line sizing §9.3, API RP 14E erosional velocity §9.4, two-phase flow-regime maps §9.5, `theory-p9`), Part X PRV sizing (§10.1–§10.7 incl. the API 526 orifice table, `theory-p10`), Part XI gas-turbine fuel estimation (§11.1–§11.3, `theory-p11`), Part XII data sources & standards (`theory-p12`). Worked-example numbers must match actual calculator output exactly.
