...
- `i18n/ja.json`, `zh.json`, `ko.json`, `th.json`, `id.json`, `ru.json`, `es.json`, `fr.json`, `de.json` — each carries the full 449-key working-tool set (368 through v2.8.1 + 36 steam keys, 19 NPSHa keys, 25 compressor keys and the `common.copiedWithUnit` toast in the v3.0 cycle) **plus** the 153 `docs.*` documentation keys (125 shipped in v2.7; 14 v2.8 blocks translated in v2.8.1; 4 steam, 4 NPSHa, 4 compressor and 2 unit-aware-copy blocks added with their translations in the same PR-2/PR-3/PR-4/PR-5 commits).
