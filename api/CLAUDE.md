...
- `flowregime.py` — two-phase flow regime map (seaborn/matplotlib server-side PNG rendering).
  ...
  G_G vs G_L) otherwise; θ = asin(Δz/L). **The gate is DIRECTIONAL (v3.7.1)** — it was `abs(θ) ≥ 30°`
...
- **flowregime** (same inputs): must classify as **Churn / Slug Flow, θ = +45.0°, vertical map**
  (j_G ≈ 0.514 m/s, j_L ≈ 0.500 m/s). *(Uphill, so v3.7.1's directional gate leaves it
  bit-identical.)*
- **flowregime downflow (v3.7.1)**: the same inputs with `elev` **negated** (θ = −45.0°) must now
